...
export default class App extends Component {
	...
	renderView = ({ index, key, ref, style, className, active, transitionState }) => {
		const activeHack = this.activeView() == index ? "transform:translateX(100%)" : "";
		return <div key={key} ref={ref} style={style} className={className} style={"min-height:100%;"+activeHack}>
	...
	renderViewByNum(index) {
		...
		switch(index) {
			...
			case 2:
				return <Effects path="/effects/:selection?" stream={stream} />
			case 3:
